...
- `crates/ui`: legacy single Dioxus app, excluded from the Cargo workspace (see the `exclude` list in the root `Cargo.toml`). Kept on disk for reference only, pending deletion — do not extend it.
